...
export interface AuditRunOptions {
  ...
  readonly execute?: (cwd: string, timeoutMs: number) => Promise<AuditAttemptResult>;
...
export async function runBunAudit(options: AuditRunOptions): Promise<AuditRunOutcome> {
  ...
  for (let attempt = 1; attempt <= maxAttempts; attempt += 1) {
    ...
    writeStderr(`bun audit transport failure; retrying attempt ${attempt + 1}/${maxAttempts}\n`);
...
async function executeBunAudit(cwd: string, timeoutMs: number): Promise<AuditAttemptResult> {
...
function parseCwd(argv: readonly string[]): string {
  ...
  const [flag, directory] = argv;
  if (argv.length === 2 && flag === "--cwd" && directory !== undefined && directory.length > 0) {
