...
- `takoserver-integration-legacy-operator-authority-retirement`: integration
  ...
  replacement identity plus Stripe settlement/checkout closure, then creates
  or exactly adopts a deterministic no-overwrite `0600` retirement checkpoint
  before any provider mutation. That checkpoint binds the capture's exact raw-file
  digest and byte length, canonical absolute-path identity digest, target,
  legacy predecessor, served source/artifact identity, and provider script
  identity without copying the path or JWK bytes. The surface re-opens the
  capture and checkpoint, re-reads the current predecessor, performs one
  `OPERATOR_PUBLIC_JWK` secret delete, and accepts only the provider-created
  direct successor. The successor may have the exact provider secret marker or
  no annotations. In either case, identical `resources.script.etag`, canonical
  source/artifact identity, every other variable/binding/secret, domain
  closure, direct history, retained checkpoint, and public product probe are
  mandatory. A successful status using that same retained evidence is the only
...
- `takoserver-integration-legacy-operator-authority-restore`: integration only
  ...
  digest from the same owned, link-free `0600` capture whose raw-file digest,
  byte length, canonical path identity, original predecessor, and Worker
  provenance exactly match the retained no-overwrite retirement checkpoint.
  Missing, ambiguous, unsafe, or mismatched evidence is refused. It sends those
  captured bytes solely through stdin to one secret put and never derives them from
...
The capture path must be absolute and outside every Git worktree. The file must
be an owned, single-link exact-`0600` regular file under an owned exact-`0700`
parent, and every path component must be symlink-free. Its nested public JWK
must be strict bounded Ed25519 public JSON. Both retirement and restore status
prove the capture's target and historical Version against live deployment
history, and expose only its digests and the explicit
`providerValueDigestVerified: false` boundary.
...
Before its provider delete, retirement creates the deterministic sidecar
`<absolute capture path>.retirement-checkpoint.json` with `O_EXCL`, exact mode
`0600`, one link, a durable file and parent-directory flush, and these exact
...
```json
{
  ...
  "workerName": "<exact target Worker name>",
  "legacyPredecessorVersionId": "<exact legacy-bearing Version uuid>",
  "sourceCommit": "<exact served 40-hex commit>",
  "bundleDigest": "sha256:<exact served artifact digest>",
  "scriptEtag": "<exact provider script identity>",
  "captureDigest": "sha256:<digest of the complete raw capture file>",
  "captureBytes": 1234,
  "capturePathDigest": "sha256:<digest of the canonical absolute capture path>",
  "publicJwkDigest": "sha256:<digest of the exact publicJwk bytes>"
}
```

`captureBytes` above is an illustrative positive integer; the retained value is
the exact raw capture-file byte length.
The sidecar contains neither the capture path nor `publicJwk`. An already
existing byte-exact checkpoint is adopted idempotently; it is never
overwritten. Retirement performs no provider call if the sidecar cannot be
retained. Restore requires this exact sidecar and re-opens both files
immediately before stdin use and after provider readback. Alternate JWK bytes,
an identical capture at another path, a different predecessor, missing or
extra checkpoint members, non-canonical checkpoint bytes, mode/link/symlink
drift, or capture/checkpoint replacement all fail closed.

One capture record owns one retirement/restore pair. After a successful restore,
a later re-retirement must create a new record at a new canonical path that
binds the same exact bytes and digest to the restored Version now selected as
the predecessor; retirement creates a new derived checkpoint for it. The old
capture/checkpoint pair is refused for that new transition. Rebinding does not
claim that Cloudflare verified the value.
...
closed. Retirement has already retained its no-overwrite checkpoint before the
provider call. Status reads the same operator capture and derived checkpoint to
bind and report their digests; a missing or mismatched checkpoint cannot be
reconstructed from another valid capture after the fact. If authoritative
status proves the selected predecessor is still current, a later explicit
apply may idempotently adopt the same checkpoint. If it proves the exact direct
successor is current, restore may consume only that retained evidence. Neither
action exposes the path or JWK bytes, and apply never blindly retries.
