...
> digest tables below are historical; see
> [Intrinsic StaticAssetBundle support](#intrinsic-staticassetbundle-support).
...
## Intrinsic StaticAssetBundle support

On 2026-09-11, the existing `StaticAssetBundle@0.1.0` package joins the
code-owned lifecycle capability set. Its schema digest remains
`sha256:83142a3f824cf1fa2793fb73f4c32203ae07fb70db789d056aebe4c0ebe9334f`
and its package digest remains
`sha256:14521f2b5a673168344ce1a1af9090a957b56948115e183b578eb449a13db392`.
No published definition, Host API lane, artifact schema, or provider mapping
changes.

The earlier exclusion grouped this resource with Forms needing an unrealized
provider supply. That rationale does not apply to the already-implemented
intrinsic artifact handler: the Host owns the tenant-scoped committed
manifest and its verified blobs. Like WorkerBundle and SQLiteMigrationSet,
StaticAssetBundle therefore requires no identity supply. Its exact package
declares `create`, `read`, `delete`, `import`, and `observe`, not `update`;
the existing package/capability/handler intersection and narrowing-only
...
historical evidence, not current pins to overwrite. Tests derive the
predecessor by excluding StaticAssetBundle from the manifest and prove that
the new capability moves semantic identity while retaining exact package
identity and self-host/public capability parity. Runtime identity continues
to be generated by the existing build path, never by a replacement literal.
Each environment still needs explicit append-only support/activation
reconvergence and functional readback; this source amendment does not claim
a deployment or live asset-serving result.
