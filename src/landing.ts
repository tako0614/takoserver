...
const landingMessages = {
  ...
  ja: {
    ...
    meterTitle: "Provision + meter",
    meterBody:
      "Deploymentをプロビジョニングし、利用量を記録して、AIの報告トークン使用量をcaptureします。",
    discovery: "Host自身のディスカバリー",
    billingTitle: "前払い・使用量ベースの課金",
    billingBody:
      "細かな単位で利用量を蓄積し、明確な集計としてウォレットへ精算します。利用量と請求額を確認できます。",
    onSuccess: "成功時",
    onFailure: "失敗時",
...
<style>
/*
 * One accent, everything else neutral, borders instead of shadows, colour only
 * where state changed — the same rule the console is built from, and the same
 * token names, so the page a person lands on and the tool they work in read as
 * one product.
 */
:root {
  color-scheme: light;
  --ink-1: #0e0e10; --ink-2: #45464b; --ink-3: #6e7076;
  --surface-0: #ffffff; --surface-1: #ffffff; --surface-2: #f6f6f7;
  --line-1: #e6e6e8; --line-2: #d3d4d7;
  --accent: #b0301f; --accent-hover: #94281a; --accent-ink: #ffffff; --mark-face: #17100f;
...
@media (prefers-color-scheme: dark) {
  :root {
    color-scheme: dark;
    --ink-1: #f2f2f3; --ink-2: #b4b5ba; --ink-3: #8a8c93;
    --surface-0: #0b0b0c; --surface-1: #101012; --surface-2: #17171a;
    --line-1: #232327; --line-2: #303036;
    --accent: #e0533c; --accent-hover: #ef6349; --accent-ink: #150907; --mark-face: #0b0b0c;
  }
}
...
.locale { display: inline-flex; padding: 3px; border: 1px solid var(--line-1); border-radius: 7px; }
.locale button {
...
.locale button[aria-pressed="true"], .locale a:hover { background: var(--surface-2); color: var(--ink-1); }
.locale a { padding: 5px 9px; color: var(--ink-3); font-size: 12px; text-decoration: none; }
.hero { margin-top: 64px; }
.eyebrow { margin: 20px 0 0; color: var(--accent); font: 600 12px/1.3 var(--mono); letter-spacing: .06em; text-transform: uppercase; }
h1 { margin: 18px 0 0; font-size: 34px; font-weight: 600; letter-spacing: -0.035em; overflow-wrap: anywhere; min-width: 0; }
p.lede { margin: 12px 0 0; color: var(--ink-2); font-size: 16px; max-width: 34rem; }
.actions { display: flex; align-items: center; gap: 10px 20px; flex-wrap: wrap; margin-top: 26px; }
.cta {
...
.shead { display: flex; align-items: center; gap: 9px; margin: 0; font-size: 13px; font-weight: 600; letter-spacing: -0.01em; }
.shead::before { content: ""; flex: none; width: 7px; height: 7px; background: var(--accent); }
.spec { margin-top: 18px; border-top: 1px solid var(--line-1); }
.spec .row { padding: 15px 0; border-bottom: 1px solid var(--line-1); }
.spec .term { display: flex; justify-content: space-between; align-items: baseline; gap: 14px; }
.spec h3 { margin: 0; font-size: 14px; font-weight: 600; }
.spec .term code { color: var(--ink-3); font-size: 12px; text-align: right; }
...
pre {
  margin: 10px 0 0; padding: 14px 16px; border: 1px solid var(--line-1);
...
.branch::before { content: "→"; color: var(--ink-3); font-family: var(--mono); font-size: 12px; }
.billing p.body, .selfhost p.body { margin: 14px 0 0; color: var(--ink-2); font-size: 14px; max-width: 34rem; }
ul { margin: 18px 0 0; padding: 0; list-style: none; border-top: 1px solid var(--line-1); }
...
li a code { overflow-wrap: anywhere; }
li span { color: var(--ink-3); font-size: 13px; white-space: nowrap; }
code { font-family: var(--mono); font-size: 13px; }
footer { margin-top: 64px; color: var(--ink-3); font-size: 13px; }
