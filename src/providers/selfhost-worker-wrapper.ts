 ...
 *
 * The service binding this module does hold reaches only the explicitly
 * allowlisted data-plane routes, and the facade behind it rewrites every
 * request completely, so a leak of the binding into tenant code buys calls
 * against the same Version's own grant — not an address, not a token, and not
 * a path anywhere else on this machine. `disallow_importable_env` is set on
 * this service besides, so `cloudflare:workers` hands the tenant an empty
 * environment.
 *
...
/** Candidate-only, opt-in VectorIndex facade. */
export const SELFHOST_WORKER_EDGE_VECTOR_BINDING_KIND = "edge.vector@0.1.0" as const;
/**
...
/** The candidate VectorIndex operation plane. */
export const SELFHOST_DATA_PLANE_VECTOR_PATH = `${SELFHOST_DATA_PLANE_PATH_PREFIX}/vector` as const;
...
const VECTOR_KIND = ${JSON.stringify(SELFHOST_WORKER_EDGE_VECTOR_BINDING_KIND)};
const VECTOR_ERROR_CODES = ["invalid_spec", "quota", "unavailable"];
        ...
        descriptor.kind === ${JSON.stringify(SELFHOST_WORKER_EDGE_KV_BINDING_KIND)}
          ...
          : descriptor.kind === ${JSON.stringify(SELFHOST_WORKER_EDGE_QUEUE_BINDING_KIND)}
            ...
            : descriptor.kind === VECTOR_KIND
              ? createVectorAdapter(call, descriptor.publicName)
...
function createPlaneCaller(rawEnv) {
  ...
  return async (url, payload, codes, project) => {
    ...
    if (!isRecord(envelope)) throw portableError("backend_unavailable");
    if (exactKeys(envelope, ["ok", "value"]) && envelope.ok === true) {
      return project === undefined ? envelope.value : project(envelope.value);
    }
    if (
      exactKeys(envelope, ["ok", "error"]) &&
      envelope.ok === false &&
 ...
 * snapshots and closes the caller's object graph before crossing the service
 * binding, then validates the response shape before handing it back.  The
 ...
 */
function createVectorAdapter(call, binding) {
  const portable = SafeObjectCreate(null);
  portable.upsert = async function (input) {
    ...
    return await vectorCall(call, binding, "upsert", snapshot, vectorUpsertOutput);
  };
  portable.get = async function (input) {
    if (arguments.length !== 1) throw vectorInvalidError();
    const snapshot = vectorInputSnapshot(vectorGetInput, input);
    return await vectorCall(call, binding, "get", snapshot, vectorGetOutput);
  };
  portable.delete = async function (input) {
    if (arguments.length !== 1) throw vectorInvalidError();
    const snapshot = vectorInputSnapshot(vectorDeleteInput, input);
    return await vectorCall(call, binding, "delete", snapshot, vectorDeleteOutput);
  };
  portable.query = async function (input) {
    if (arguments.length !== 1) throw vectorInvalidError();
    const snapshot = vectorInputSnapshot(vectorQueryInput, input);
    return await vectorCall(call, binding, "query", snapshot, vectorQueryOutput);
  };
  return SafeApply(SafeObjectFreeze, SafeObject, [portable]);
}

const VECTOR_MISSING = SafeSymbol("takoserver-selfhost-vector-missing");
...
async function vectorCall(call, binding, operation, input, project) {
  let payload;
  try {
    payload = planeRequest(binding, operation);
    if (!vectorIsRecord(payload)) throw vectorInvalidError();
    payload.input = input;
    SafeApply(SafeObjectFreeze, SafeObject, [payload]);
  } catch {
