 ...
 *
 * Production constructs this module only inside the named route-less provider
 * executor (ADR 0001); the public API Worker imports only its credential-free
 * proxy. The token must stay scoped — Workers Scripts, R2, D1, and the
 * configured zones only — because the blast radius of the executor isolate is
 * exactly what that token can do.
 *
  ...
  async #applyWorkerVersion(input: ApplyInput): Promise<ProviderTicket> {
    ...
    if (
      bindings.some((binding) => {
        ...
        return name !== undefined && WORKER_VERSION_INTERNAL_BINDING_NAMES.has(name);
      }) ||
      requiredSensitive.some((name) => WORKER_VERSION_INTERNAL_BINDING_NAMES.has(name))
    ) {
