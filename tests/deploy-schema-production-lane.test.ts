...
// 0001-0052 source. Historical 0001-0049 fixtures are passed explicitly by
...
describe("production-shaped D1 migration lane", () => {
  ...
  test("integration protected selectors refuse the unreviewed current migration tail", async () => {
    ...
    try {
      ...
      expect(failure).toBeInstanceOf(DeployError);
      expect(String(failure)).toContain("exact audited source inventory 0001-0052");
  ...
  test("a fixed wave refuses migrations outside the exact audited 0001-0052 inventory", async () => {
    ...
    try {
      ...
      copyFileSync(
        ...
        join(migrationDirectory, "0053_unreviewed_extension.sql"),
      );
      const failure = await runD1Schema(
        {
          action: "status",
  ...
  test("the thirteen no-overwrite wave receipts cover the exact 30-file production suffix once", async () => {
    ...
    try {
      ...
      expect(new Set(receipted.map(({ name }) => name)).size).toBe(30);
    } finally {
      rmSync(root, { recursive: true, force: true });
    }
  });
