...
const INVENTED_UNAUDITED_TAIL = [
  [
    "0063_container_runtime_input_custody.sql",
    "CREATE TABLE synthetic_0063_container_runtime_input_custody (id TEXT);\n",
  ],
  [
    "0064_container_runtime_input_rewrap.sql",
    "CREATE TABLE synthetic_0064_container_runtime_input_rewrap (id TEXT);\n",
  ],
  [
    "0065_container_runtime_input_acceptance.sql",
    "CREATE TABLE synthetic_0065_container_runtime_input_acceptance (id TEXT);\n",
  ],
] as const;
...
// 0001-0062 source. Historical 0001-0049 fixtures are passed explicitly by
...
describe("production-shaped D1 migration lane", () => {
  ...
  test("integration protected selectors refuse the unreviewed current migration tail", async () => {
    ...
    try {
      ...
      expect(failure).toBeInstanceOf(DeployError);
      expect(String(failure)).toContain("exact audited source inventory 0001-0062");
  ...
  test("a fixed wave refuses migrations outside the exact audited 0001-0062 inventory", async () => {
    ...
    try {
      ...
      copyFileSync(
        ...
        join(migrationDirectory, "0063_unreviewed_extension.sql"),
      );
      const failure = await runD1Schema(
        {
          action: "status",
      ...
      expect(failure.message).toContain("exact audited source inventory 0001-0062");
    } finally {
      rmSync(root, { recursive: true, force: true });
    }
  });

  for (const refusedBoundary of ["0058", "0059", "0060", "0061", "0062"] as const) {
  ...
  test("the eighteen no-overwrite wave receipts cover the exact 35-file production suffix once", async () => {
    ...
    try {
      ...
      expect(new Set(receipted.map(({ name }) => name)).size).toBe(35);
    } finally {
      rmSync(root, { recursive: true, force: true });
    }
  });
