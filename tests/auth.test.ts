...
async function ownerWithOrganization() {
  ...
  const organization = await accounts.createOrganization({ actor, name: "Acme" });
  return { principal, sessionToken, actor, organization };
}

describe("accounts", () => {
  test("returns the same principal for a repeated sign-in", async () => {
    const first = await accounts.signIn({ provider: "google", assertion: "owner" });
    const second = await accounts.signIn({ provider: "google", assertion: "owner" });
    ...
    const other = await accounts.signIn({ provider: "github", assertion: "owner" });
  ...
  test("hands out an API key secret exactly once and keeps only its digest", async () => {
    ...
    expect(resolved).toMatchObject({ organizationId: organization.id, kind: "api_key" });
  ...
  test("stops accepting a key once revoked or expired", async () => {
    ...
    now -= 61_000;
    await accounts.revokeApiKey({ actor, organizationId: organization.id, apiKeyId: apiKey.id });
    expect(await accounts.authenticate(`Bearer ${secret}`)).toBeNull();
  });

  test("refuses to act on an organization the caller does not own", async () => {
    const { organization } = await ownerWithOrganization();
    const intruder = await accounts.signIn({ provider: "github", assertion: "intruder" });
