...
function result(overrides: Partial<AuditAttemptResult> = {}): AuditAttemptResult {
  return {
    exitCode: 0,
    stdout: "{}\n",
    stderr: "",
...
test("classifies only failed transport outcomes as transient", () => {
  expect(
    isTransientAuditFailure(result({ exitCode: 1, stdout: "", stderr: "ConnectionClosed" })),
  ).toBe(true);
  expect(isTransientAuditFailure(result({ exitCode: 1, stdout: "", timedOut: true }))).toBe(true);
  expect(isTransientAuditFailure(result({ exitCode: 0, stdout: "", timedOut: true }))).toBe(true);
  expect(
    isTransientAuditFailure(result({ exitCode: 1, stdout: '{"vulnerabilities":{"high":1}}' })),
  ).toBe(false);
  expect(
    isTransientAuditFailure(
      result({
        exitCode: 1,
        stdout: '{"vulnerabilities":{"high":1},"advisory":"network error in package text"}',
      }),
    ),
  ).toBe(false);
  expect(isTransientAuditFailure(result({ exitCode: 0, stderr: "network error" }))).toBe(false);
});
...
test("retries a transport failure with bounded backoff", async () => {
  ...
  const outcome = await runBunAudit({
    ...
    execute: async () => {
      ...
      if (attempt === undefined) throw new Error("unexpected extra audit attempt");
      return attempt;
    },
    sleep: async (delay) => {
      delays.push(delay);
  ...
  expect(stderr).toEqual(["bun audit transport failure; retrying attempt 2/3\n"]);
});
...
test("never retries an authoritative vulnerability result", async () => {
  ...
  const outcome = await runBunAudit({
    ...
    execute: async () => {
      ...
      return result({
        exitCode: 1,
        stdout: '{"vulnerabilities":{"critical":1},"advisory":"network error"}\n',
      });
    },
    sleep: async () => {
...
test("exhausted timeouts fail with 124 and never become green", async () => {
  ...
  expect(stderr.at(-1)).toBe("bun audit timed out after 10ms (attempt 3/3)\n");
});
...
test("keeps registry audits on dependency changes and scheduled cadence", async () => {
  ...
  expect(qualityWorkflow).not.toContain("bun audit");
  expect(qualityWorkflow).not.toContain("run-bun-audit.ts");

  expect(dependencyWorkflow).toContain('cron: "41 3 * * *"');
  ...
  expect(dependencyWorkflow).toContain('git diff --name-only -z --no-renames "$range"');
  ...
  expect(dependencyWorkflow.match(/if: steps\.scope\.outputs\.required == 'true'/gu)?.length).toBe(
    2,
  );
