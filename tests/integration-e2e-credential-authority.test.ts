...
describe("integration-only exact-organization API-key authority", () => {
  ...
  test("refuses a wrong organization, scope, or TTL before API-key storage", async () => {
    ...
    const variants = [
      ...
      { scopes: ["resources:write", "resources:read"] },
      { ttlSeconds: 0 },
      { ttlSeconds: 3_601 },
    ] as const;
  ...
  test("derives a deterministic ID, never returns a replayed secret, and permits one present key", async () => {
    ...
    const expectedId = await deterministicIntegrationE2eApiKeyId(operationId);

    const first = await fixture.call("issue", operationId);
    expect(first.response.status).toBe(201);
    expect(record(await first.response.json())).toMatchObject({ keyId: expectedId });
    ...
    expect(await count(fixture.sql, "auth_tokens", "kind = 'api_key'")).toBe(1);
    ...
    expect(expired).toMatchObject({ keyId: expectedId, present: true, usable: false });
  ...
  test("status is signed and revoke is exact, immediate, and idempotent", async () => {
    ...
    const secret = string(issue.secret);
